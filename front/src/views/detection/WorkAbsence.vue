<template>
  <div class="work-absence-management">
    ...
    <ElCard class="main-card">
      ...
      <ElTable :data="areaList" style="width: 100%">
        ...
        <ElTableColumn prop="absenceThreshold" label="脱岗阈值(秒)" />
    ...
    >
      <div class="dialog-content">
        ...
        >
          ...
          <ElFormItem label="脱岗阈值(秒)" prop="absenceThreshold">
            <ElInput-number
              v-model="formData.absenceThreshold"
              :min="5"
              :max="300"
              :step="5"
              placeholder="请输入脱岗阈值"
...
import { getWorkAbsenceAreaList, createWorkAbsenceArea, updateWorkAbsenceArea, deleteWorkAbsenceArea, toggleWorkAbsenceAreaStatus } from '@/api/detection'
import { getCameraList } from '@/api/system'

interface WorkAbsenceArea {
  id: number
  cameraId: number
  cameraName: string
  machineName: string
...
const areaList = ref<WorkAbsenceArea[]>([])
...
const formRules = {
  ...
  absenceThreshold: [
    { required: true, message: '请输入脱岗阈值', trigger: 'blur' },
    { type: 'number', min: 5, message: '脱岗阈值不能小于5秒', trigger: 'blur' }
  ],
  areaPoints: [
...
async function loadAreaList() {
  try {
    const response = await getWorkAbsenceAreaList({
      ...searchForm,
...
// 编辑区域
function handleEdit(row: WorkAbsenceArea) {
  dialogTitle.value = '编辑脱岗区域'
  isEdit.value = true
  formData.id = row.id
  ...
  formData.absenceThreshold = row.absenceThreshold
...
// 删除区域
async function handleDelete(row: WorkAbsenceArea) {
  try {
    ...
    await deleteWorkAbsenceArea(row.id)
...
// 切换区域状态
async function handleToggleStatus(row: WorkAbsenceArea) {
  try {
    await toggleWorkAbsenceAreaStatus(row.id, row.status === 1 ? 0 : 1)
...
async function handleSubmit() {
  ...
  try {
    ...
    if (isEdit.value) {
      await updateWorkAbsenceArea(formData)
    } else {
      await createWorkAbsenceArea(formData)
    }
...
function resetFormData() {
  ...
  formData.areaPoints = ''
  formData.absenceThreshold = 30
  formData.sensitivity = 7
  formData.checkInterval = 5
}
